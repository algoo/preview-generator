...
class OfficePreviewBuilderLibreofficeOnline(DocumentPreviewBuilder):
    weight = 240

    @classmethod
    def get_label(cls) -> str:
        return "Documents - based on LibreOfficeOnline"
    ...
    def convert_office_document_to_pdf(
        ...
        with create_flag_file(output_filepath):
            ...
            if not os.path.exists(output_filepath):
                ...
                files = {"file": open(temporary_input_content_path, "rb")}
                response = requests.post(
                    url, files=files, verify=False, data={"FullSheetPreview": "true"}
                )
                with open(output_filepath, "wb+") as pdf_handle:
