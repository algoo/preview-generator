...
ODP_FILE_PATH = os.path.join(CURRENT_DIR, "the_odp.odp")
ODP_FILE_PATH_NO_EXTENSION = os.path.join(CURRENT_DIR, "the_odp")
FILE_HASH = hashlib.md5(ODP_FILE_PATH.encode("utf-8")).hexdigest()
ODP_FILE_EXT = ".odp"
...
def test_to_jpeg() -> None:
    ...
    path0 = manager.get_jpeg_preview(
        file_path=ODP_FILE_PATH, height=512, width=256, page=0, force=True
    )
    ...
    path1 = manager.get_jpeg_preview(
        file_path=ODP_FILE_PATH, height=512, width=256, page=1, force=True
    )
...
def test_odp_to_jpeg_no_extension() -> None:
    manager = PreviewManager(cache_folder_path=CACHE_DIR, create_folder=True)
    assert (
        manager.has_jpeg_preview(file_path=ODP_FILE_PATH_NO_EXTENSION, file_ext=ODP_FILE_EXT)
        is True
    )
    path0 = manager.get_jpeg_preview(
...
def test_to_jpeg_no_size() -> None:
    manager = PreviewManager(cache_folder_path=CACHE_DIR, create_folder=True)
    assert manager.has_jpeg_preview(file_path=ODP_FILE_PATH) is True
    path_to_file = manager.get_jpeg_preview(file_path=ODP_FILE_PATH, page=0, force=True)
...
def test_to_jpeg_no_page() -> None:
    ...
    path_to_file = manager.get_jpeg_preview(
        file_path=ODP_FILE_PATH, height=512, width=512, force=True
    )
...
def test_to_jpeg_no_size_no_page() -> None:
    manager = PreviewManager(cache_folder_path=CACHE_DIR, create_folder=True)
    assert manager.has_jpeg_preview(file_path=ODP_FILE_PATH) is True
    path_to_file = manager.get_jpeg_preview(file_path=ODP_FILE_PATH, force=True)
    assert os.path.exists(path_to_file) is True
    assert os.path.getsize(path_to_file) > 0
    assert re.match(test_utils.CACHE_FILE_PATH_PATTERN__JPEG, path_to_file)
...
def test_to_pdf_full_export() -> None:
    ...
    path_to_file = manager.get_pdf_preview(file_path=ODP_FILE_PATH, page=-1, force=True)
    assert os.path.exists(path_to_file) is True
    assert os.path.getsize(path_to_file) > 0
...
def test_to_pdf_one_page() -> None:
    manager = PreviewManager(cache_folder_path=CACHE_DIR, create_folder=True)
    assert manager.has_pdf_preview(file_path=ODP_FILE_PATH) is True
    path_0 = manager.get_pdf_preview(file_path=ODP_FILE_PATH, page=0, force=True)
...
def test_to_text() -> None:
    manager = PreviewManager(cache_folder_path=CACHE_DIR, create_folder=True)
    assert manager.has_text_preview(file_path=ODP_FILE_PATH) is False
    with pytest.raises(UnavailablePreviewType):
        manager.get_text_preview(file_path=ODP_FILE_PATH, force=True)


def test_to_json() -> None:
    manager = PreviewManager(cache_folder_path=CACHE_DIR, create_folder=True)
    assert manager.has_json_preview(file_path=ODP_FILE_PATH) is True
    manager.get_json_preview(file_path=ODP_FILE_PATH, force=True)
...
def test_to_pdf() -> None:
    manager = PreviewManager(cache_folder_path=CACHE_DIR, create_folder=True)
    assert manager.has_pdf_preview(file_path=ODP_FILE_PATH) is True
    manager.get_pdf_preview(file_path=ODP_FILE_PATH, force=True)
    # TODO - G.M - 2018-11-06 - To be completed
...
def test_get_nb_page_no_extension() -> None:
    manager = PreviewManager(cache_folder_path=CACHE_DIR, create_folder=True)
    nb_page = manager.get_page_nb(file_path=ODP_FILE_PATH_NO_EXTENSION, file_ext=ODP_FILE_EXT)
    assert nb_page == 5
